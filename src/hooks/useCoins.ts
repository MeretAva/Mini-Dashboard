...
export function useCoins() { 
    ...
    useEffect(() => { 
        ...
        const fetchCoins = async () => { 
            try { 
                const res = await fetch( 'https://api.coingecko.com/api/v3/coins/markets?vs_currency=eur&order=market_cap_desc&per_page=50&page=1&sparkline=false&price_change_percentage=24h'   
                )
        ...
        fetchCoins() }, []) // Empty dependency array means this runs once when the component first mounts
