...
function SortIcon({ colKey, sortKey, sortDir }: SortIconProps) {
  if (sortKey !== colKey) return (
    ...
    <svg className="inline w-3 h-3 ml-1 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16V4m0 0L3 8m4-4l4 4M17 8v12m0 0l4-4m-4 4l-4-4" />
    </svg>
  )

  return sortDir === 'asc' ? (
    <svg className="inline w-3 h-3 ml-1 text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function CoinTable({ coins, loading, error, selectedCoinId, onSelectCoin }: Props) {
  const [search, setSearch] = useState('')
  const [filter, setFilter] = useState<Filter>('all')
  const [sortKey, setSortKey] = useState<SortKey>('market_cap')
  const [sortDir, setSortDir] = useState<SortDir>('asc')

  // Derive the displayed list from raw coins data
  const displayed = useMemo(() => {
    let result = [...coins]

    // 1. Filter by search query — matches against name or symbol
  ...
  // If the same column header is clicked again, flip the direction.
  ...
  // Format as EUR currency using German locale
  const formatPrice = (n: number) =>
    n.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })

  // Abbreviate large market cap numbers for readability
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-gray-500 border-b">
            ...
            <th className="pb-2 cursor-pointer" onClick={() => handleSort('price')}>
              Price<SortIcon colKey='price' sortKey={sortKey} sortDir={sortDir} />
            </th>
            <th className="pb-2 cursor-pointer" onClick={() => handleSort('change')}>
              24h<SortIcon colKey='change' sortKey={sortKey} sortDir={sortDir} />
            </th>
            <th className="pb-2 cursor-pointer" onClick={() => handleSort('market_cap')}>
              Market Cap<SortIcon colKey='market_cap' sortKey={sortKey} sortDir={sortDir} />
            </th>
        ...
        <tbody>
          ...
          {displayed.length === 0 && (
            <tr><td colSpan={5} className="text-center py-8 text-gray-400">No results found</td></tr>
          )}
          {displayed.map(coin => (
            <tr
              key={coin.id}
